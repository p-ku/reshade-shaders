...
uniform float Chromatic_Aberration_Intensity <
    __UNIFORM_SLIDER_FLOAT1 ui_label = "Intensity";
ui_min = 0.0;
ui_max = 1.0;
ui_category = "Chromatic Aberration";
> = 0.25;
uniform uint Chromatic_Aberration_Samples < __UNIFORM_SLIDER_INT1 ui_label =
    "Samples";
ui_min = 4;
ui_max = 32;
ui_step = 4;
...
#if VIGNETTE
uniform float Vignette_Intensity < __UNIFORM_SLIDER_FLOAT1 ui_label =
    "Intensity";
ui_min = 0.0;
ui_max = 1.0;
...
#if FILM_GRAIN
uniform float Film_Grain_Intensity < __UNIFORM_SLIDER_FLOAT1 ui_label =
    "Intensity";
ui_min = 0.0;
ui_max = 1.0;
...
float3 nrand3(float2 seed, float tr) {
  return frac(sin(dot(seed.xy, float2(34.483, 89.637) * tr)) *
              float3(29156.4765, 38273.5639, 47843.7546));
}
#endif
...
float4 PerfectPerspective_PS(float4 pixelPos : SV_Position,
  ...
  float blue_distance =
      center_distance * cos(0.25 * PI * center_distance *
                            Chromatic_Aberration_Intensity / max_diagonal);
...
// Jitter
  ...
  const float center_angle =
      atan(Vignette_Intensity * center_distance / max_diagonal);
  ...
  const float luma_factor =
      max(Film_Grain_Intensity * (luma - luma * luma), 0.0);

// add noise to luma
#if COLOR_FILM_GRAIN
  const float tr = frac(timer / 1337.7331);
  const float3 grain_noise =
      pkuBlueNoise::triangle(2.0 * nrand3(texCoord, tr) - 1.0);
  const float3 new_luma = luma + luma_factor * grain_noise;
#else
  const float tr = frac(timer / 1337.7331);
  const float grain_noise =
      pkuBlueNoise::triangle(2.0 * nrand(texCoord + tr) - 1.0);
  const float new_luma = luma + luma_factor * grain_noise;
#endif
  display.rgb = display.rgb * new_luma / luma;
#endif
  display = pkuBlueNoise::dither(display, uint2(pixelPos.xy), timer + 500u);
  return display;
}
