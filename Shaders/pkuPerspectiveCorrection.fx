...
uniform uint FovAngle < __UNIFORM_SLIDER_INT1 ui_category_closed = true;
ui_units = "°";
...
uniform uint FovType < __UNIFORM_COMBO_INT1 ui_label = "Field of view type";
ui_items = "horizontal\0"
           "vertical\0";
> = 1u;

uniform float K < __UNIFORM_SLIDER_FLOAT1 ui_category_closed = true;
ui_units = " k";
...
uniform float CroppingFactor < __UNIFORM_SLIDER_FLOAT1 ui_label = "Cropping";
ui_min = 0f;
ui_max = 1f;
ui_step = 0.005;
> = 0.0;
...
/* Azimuthal spherical perspective projection equations © 2022 Jakub Maksymilian
   Fober These algorithms are part of the following scientific papers: ·
   arXiv:2003.10558 [cs.GR] (2020) · arXiv:2010.04077 [cs.GR] (2020) */
float get_radius(float theta, float rcp_f, float k) // get image radius
{
  return tan(abs(k) * theta) / rcp_f /
         abs(k); // stereographic, rectilinear projections
}
...
void PerfectPerspective_VS(in uint vertexId : SV_VertexID,
  ...
  static const float croppingHorizontal =
      get_radius(atan(tan(halfOmega) / radiusOfOmega * viewProportions.x),
                 rcp_focal, K) /
      viewProportions.x;

  // Vertical point radius
  static const float croppingVertical =
      get_radius(atan(tan(halfOmega) / radiusOfOmega * viewProportions.y),
                 rcp_focal, K) /
      viewProportions.y;
  // Diagonal point radius
  static const float anamorphicDiagonal =
      length(float2(viewProportions.x, viewProportions.y));

  // Circular fish-eye
  static const float circularFishEye =
      max(croppingHorizontal, croppingVertical);
  // Cropped circle
  static const float croppedCircle = min(croppingHorizontal, croppingVertical);
  // Full-frame
  static const float fullFrame =
      get_radius(atan(tan(halfOmega) / radiusOfOmega * anamorphicDiagonal),
                 rcp_focal, K) /
      anamorphicDiagonal;

  // Get radius scaling for bounds alignment
  static const float croppingScalar =
      lerp(circularFishEye, fullFrame, CroppingFactor);
...
float4 PerfectPerspective_PS(float4 pixelPos : SV_Position,
  ...
  static const float2 toUvCoord =
      radiusOfOmega / (tan(halfOmega) * viewProportions);
  ...
  float4 display = pkuBlueNoise::dither(tex2D(ReShade::BackBuffer, texCoord),
                                        uint2(pixelPos.xy), timer);
